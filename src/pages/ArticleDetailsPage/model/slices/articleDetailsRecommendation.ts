...
import { type IArticleRecommendationState } from '../types';

const recommendationAdapter = createEntityAdapter<IArticle>();

export const getArticleRecommendation = recommendationAdapter.getSelectors<IStateSchema>(
    (state) => state.articleRecommendation || recommendationAdapter.getInitialState(),
);

export const { reducer: articleDetailsRecommendationReducer, actions: articleDetailsRecommendationSliceActions } = createSlice({
