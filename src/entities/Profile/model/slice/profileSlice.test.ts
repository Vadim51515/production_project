...
describe(' profileSlice', () => {
    ...
    test('fetchProfileData.rejected изменяет ошибку и статус', () => {
        expect(profileReducer(
            state,
            fetchProfileData.rejected(null, '', '', 'Тестовая ошибка')
        )).toStrictEqual({
            ...state,
            status: RuntimeStatuses.Error,
            error: 'Тестовая ошибка'
    ...
    test(
        ...
        () => {
            expect(profileReducer(
                state,
                updateProfileData.rejected(null, '', '', 'Тестовая ошибка')
            )).toStrictEqual({
                ...state,
                status: RuntimeStatuses.Error,
                error: 'Тестовая ошибка'
            ...
            expect(profileReducer(
                state,
                updateProfileData.rejected(null, '', '', { firstName: ErrorsStatuses.FieldRequired })
            )).toStrictEqual({
                ...state,
                status: RuntimeStatuses.Error,
