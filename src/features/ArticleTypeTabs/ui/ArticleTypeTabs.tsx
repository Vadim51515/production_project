...
export const ArticleTypeTabs: FC<IArticleTypeTabsProps> = () => {
    ...
    const { setPage, setType, fetchArticleList } = useActions(articlesPageActions);

    const typeTubs = useMemo(
        () =>
            Object.keys(ArticleType).map((label) => ({
                label,
    ...
    return (
        <Tabs
            tabs={typeTubs as TOptions<ArticleType>}
            value={type}
