...
describe('classNames', () => {
    ...
    test("С основным, модами, и  дополнительными параметрами'", () => {
        ...
        expect(classNames('someClass', { hovered: true, scrollable: true, disabled: false }, ['class1', 'class2'])).toBe(
            expectedValue,
        );
    });
    ...
    test('Все переданные моды undefined', () => {
        const expectedValue = 'someClass class1 class2';
        expect(
            classNames('someClass', { hovered: undefined, scrollable: undefined, disabled: undefined }, ['class1', 'class2']),
        ).toBe(expectedValue);
    });
});
